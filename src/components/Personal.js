...
export default class Personal extends Component {
    ...
    fetchRoutines = async () => {
        ...
        await axios.get(`users/${username}/routines`)
        .then(
    ...
    showEditRoutine = (can_show) => {
        this.setState({ canShowEditRoutine: can_show, canShowEditActivity:false})
    }
    ...
    removeRoutineActivity = (id) => {
        axios.delete(`/routine_activities/${this.state.selected_routine_id}` + id,{headers: getHeaders()})
        .then(() => {
            this.setState({ routineActivities: this.state.routineActivities.filter(activity => activity.id !== id) })
        })
        .catch(err => console.log(err))
    ...
    printRoutinesAsRows = (routines = [], counter) => {
        ...
        let end_index = (possible_end_index > routines.length) ? routines.length : possible_end_index;
        ...
        return routines_to_show.map((routine, index) => {
            return <tr>
                ...
                <td>{routine.name}</td>
                <td>{routine.goal}</td>
                <td><button className="btn btn-primary" onClick={() => this.setRoutineActivities(routine.activities,routine.id)}>{routine.activities.length}</button></td>
    ...
    printActivitiesAsRows = () => {
        return this.state.routineActivities.map((activity, index) => {
            return <tr>
                ...
                <td><button className='btn btn-primary btn-sm' onClick={()=>this.showEditRoutine(!this.state.canShowEditRoutine)}>Edit Routine</button></td>               
            </tr>
        })
    }
    ...
    addtoRoutine = async (activity_id, count, duration) =>{
        ...
        let username = localStorage.getItem('username');
        await axios.post(url, {activityId:activity_id, count: this.state.count, duration:this.state.duration},{
            headers: getHeaders()
        })

        //get all the routines
        await axios.get(`users/${username}/routines`,{headers: getHeaders()}).then(res =>{

            //filter for the selected routine
            let [routine] = res.data.filter(routine => routine.id = this.state.selected_routine_id)
    ...
    render() {
            ...
            return (
                <div>
                    <div>
                        ...
                        <button className="btn btn-outline-success btn-sm" onClick={this.clearRoutineActivities}>Go Back</button>
                        
                    </div>

                    <div className="activities_list">

                        <Activities addtoRoutine={this.addtoRoutine} setCount={this.setCount} setDuration={this.setDuration} selected_routine_id={this.selected_routine_id}></Activities>
        ...
        return (
            <div className="routines_section">
                <h2 className='list-title'>My Routines: {this.state.routines.length}</h2>
                ...
                <table className="routines_table">
                    ...
                    <tfoot className="d-flex">
                        ...
                        <button className="btn btn-outline-success btn-sm" onClick={() => this.clearRoutineActivities()}>Clear</button>
                    </tfoot>
                </table>
            </div>)
